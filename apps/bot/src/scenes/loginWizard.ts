 ...
 * LoginHUB). O wizard não fala HTTP com o LoginHub: quem conhece as rotas, os
 * três desfechos do login e a diferença entre TOTP e código de recuperação é o
 * kit — aqui só mora a conversa.
 */
...
async function concluirVinculo(ctx: BotContext, session: HubSessionData) {
  ...
  await ctx.reply(
    `✅ <b>Conta vinculada com sucesso!</b>\n\n` +
      `Bem-vindo(a) ao NotesAPP Bot, ${dono.nome ?? ctx.from?.first_name ?? ''}! 🎉\n` +
      'Suas notas do site e do bot agora andam juntas.\n\n' +
      'O que deseja fazer?',
...
export const loginWizard = new Scenes.WizardScene<BotContext>(
  ...
  //
  // O hub responde 200 em TRÊS desfechos e só um traz sessão. Este passo tratava
  // apenas o primeiro (e ainda olhava `requirePasswordChange`, flag que o hub
  // não devolve desde que a senha passou a se definir pelo magic link): com o
  // 2FA exigido de toda conta, o normal virou desfecho sem token e o login do
  // bot parou de funcionar. Quem separa os três agora é o kit.
  async (ctx: BotContext) => {
    if (await handleCancel(ctx)) return;
    if (!ctx.message || !('text' in ctx.message)) return;
      ...
      if (err instanceof HubApiError && err.status === 401) {
        await ctx.reply(
          '❌ <b>Credenciais inválidas.</b>\n\nDigite a sua <b>senha</b> novamente, ou cancele e confira se ' +
            'você já definiu a senha pelo link do e-mail de convite em https://notes.astralwavelabel.com.',
          { parse_mode: 'HTML', ...cancelKeyboard }
        );
  ...
  async (ctx: BotContext) => {
    ...
    if (!challengeToken) {
      await ctx.reply('⚠️ A janela de verificação expirou. Envie /start para começar de novo.');
      return ctx.scene.leave();
    }
